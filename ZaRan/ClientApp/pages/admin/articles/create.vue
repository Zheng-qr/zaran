  ...
  <div>
    ...
    <div class="sm:flex sm:items-center sm:justify-between mb-6">
      <div>
        ...
        <p class="mt-2 text-sm text-gray-700">创建新的文章内容</p>
      </div>
...
import type { ArticleCreateRequest } from '~/api'
...
const saveArticle = async (status: number) => {
  ...
  try {
    ...
    const response = await ArticlesService.articlePostEndpoint(articleData)
    
    const { success } = useGlobalNotifications()
    success(
      status === ARTICLE_STATUS.PUBLISHED ? '文章发布成功' : '文章保存成功',
      status === ARTICLE_STATUS.PUBLISHED ? '文章已成功发布' : '文章已保存为草稿'
    )
